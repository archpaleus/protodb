...
#define AK_TOOLKIT_STRING_VIEW_OPERATIONS()                    \
  operator ::std::string_view() const {                        \
    return AK_TOOLKIT_STRING_REF_RETURN_TYPE(c_str(), size()); \
  }
...
// # size_tag and segment_tag are used to enable constant int inference in
...
template <typename Tag1, typename Tag2>
struct concat_tag {
  typedef typename select_conctenated_array_tag<
      typename array_upgrade<Tag1>::type,
...
// # A wraper over a string literal with alternate interface. No ownership
// management
...
// # A function that converts raw string literal into string_literal and deduces
// the size.
...
class string<N, char_array> {
  ...
  constexpr explicit string(private_ctor, string<M, TL> const& l,
                            string<N - M, TR> const& r,
                            detail::int_sequence<Il...>,
                            detail::int_sequence<Ir...>)
      : _array{l[Il]..., r[Ir]..., 0} {}

  template <typename T, int... Il>
  constexpr explicit string(private_ctor, T const& l,
                            detail::int_sequence<Il...>, int offset)
      : _array{l[Il + offset]..., 0} {}

 public:
  template <int M, typename TL, typename TR,
            typename std::enable_if<(M <= N), bool>::type = true>
  constexpr explicit string(string<M, TL> l, string<N - M, TR> r, int, int)
      : string(private_ctor{}, l, r, detail::make_int_sequence<M>{},
               detail::make_int_sequence<N - M>{}) {}

  template <int N2_plus_1, int from>
  constexpr explicit string(const char (&lit)[N2_plus_1], size_tag<from>)
      : string(private_ctor{}, lit,
               detail::make_int_sequence<N2_plus_1 - 1 - from>{}, from) {}
  ...
  constexpr explicit string(const char (&lit)[N2_plus_1],
                            segment_tag<from, len>)
...
// # This implements a suffix of a null-terminated array that stores elements on
// stack.
...
class string<N, array_suffix> {
  ...
  template <int M, typename TL, typename TR>
  constexpr static char permute1(string<M, TL> const& l,
                                 string<N - M, TR> const&, int lsize, int rsize,
                                 int i) {
    return i < M - lsize + N - M - rsize ? '\0'
                                         : l[i - (M - lsize + N - M - rsize)];
  }

  template <int M, typename TL, typename TR>
  constexpr static char permute2(string<M, TL> const& l,
                                 string<N - M, TR> const& r, int lsize,
                                 int rsize, int i) {
    return i < N - M - rsize ? l[lsize - (N - M - rsize) + i]
                             : r[i - (N - M - rsize)];
  }
  ...
  constexpr explicit string(private_ctor, string<M, TL> const& l,
                            string<N - M, TR> const& r,
                            detail::int_sequence<Il...>,
                            detail::int_sequence<Ir...>, int lsize, int rsize)
      : _array{permute1(l, r, lsize, rsize, Il)...,
               permute2(l, r, lsize, rsize, Ir)..., 0},
        _offset(N - lsize - rsize) {}

  template <int... Il, typename T>
  constexpr explicit string(private_ctor, T const& l,
                            detail::int_sequence<Il...>, int offset)
      : _array{l[Il + offset]..., 0}, _offset(offset) {}

 public:
  template <int M, typename TL, typename TR,
            typename std::enable_if<(M <= N), bool>::type = true>
  constexpr explicit string(string<M, TL> l, string<N - M, TR> r, int lsize,
                            int rsize)
      : string(private_ctor{}, l, r, detail::make_int_sequence<M>{},
               detail::make_int_sequence<N - M>{}, lsize, rsize) {}
...
class string<N, literal_suffix> {
 ...
 public:
  ...
  constexpr string(const char (&lit)[N + 1])
      : _lit((AK_TOOLKIT_ASSERT(lit[N] == 0), lit)), _offset(0) {}
  constexpr char operator[](int i) const {
    return AK_TOOLKIT_ASSERT(i >= 0 && i < int(size())), _lit[i + _offset];
  }
  AK_TOOLKIT_STRING_VIEW_OPERATIONS()
  constexpr ::std::size_t size() const {
    return N - _offset;
...
constexpr string_literal_suffix<N> suffix(string_literal<N> l, int offset) {
  return AK_TOOLKIT_ASSERT(0 <= offset && offset <= N),
         string_literal_suffix<N>(l, offset);
}

// # A function that converts raw string literal + offset into string_literal
// and deduces the size.

template <int OFFSET, int N_PLUS_1,
          typename std::enable_if<(OFFSET >= 0 && OFFSET < N_PLUS_1),
                                  bool>::type = true>
constexpr array_string<N_PLUS_1 - 1 - OFFSET> offset_literal(
    const char (&lit)[N_PLUS_1]) {
  return AK_TOOLKIT_ASSERT(lit[N_PLUS_1 - 1] == 0),
         array_string<N_PLUS_1 - 1 - OFFSET>(lit, size_tag<OFFSET>{});
}
...
void offset_literal(const char (&)[N_PLUS_1]) {
  static_assert(OFFSET >= 0 && OFFSET < N_PLUS_1,
                "bad offset provided to offset_literal");
}

template <int OFFSET, int LEN, int N_PLUS_1,
          typename std::enable_if<(OFFSET >= 0 && OFFSET < N_PLUS_1),
                                  bool>::type = true,
          typename std::enable_if<(LEN >= 0 && OFFSET + LEN < N_PLUS_1),
                                  bool>::type = true>
constexpr array_string<LEN> substr(const char (&lit)[N_PLUS_1]) {
  return AK_TOOLKIT_ASSERT(lit[N_PLUS_1 - 1] == '\0'),
         array_string<LEN>(lit, segment_tag<OFFSET, LEN>{});
}

template <
    int OFFSET, int LEN, int N, typename T,
...
// # A set of concatenating operators, for different combinations of raw
// literals, string_literal<>, and array_string<>

template <int N1, int N2, typename TL, typename TR>
constexpr string<N1 + N2, typename concat_tag<TL, TR>::type> operator+(
    string<N1, TL> const& l, string<N2, TR> const& r) {
  return string<N1 + N2, typename concat_tag<TL, TR>::type>(l, r, l.size(),
                                                            r.size());
}

template <int N1_1, int N2, typename TR>
constexpr string<N1_1 - 1 + N2, typename array_upgrade<TR>::type> operator+(
    const char (&l)[N1_1], string<N2, TR> const& r) {
  return string<N1_1 - 1 + N2, typename array_upgrade<TR>::type>(
      string_literal<N1_1 - 1>(l), r, N1_1 - 1, r.size());
}

template <int N1, int N2_1, typename TL>
constexpr string<N1 + N2_1 - 1, typename array_upgrade<TL>::type> operator+(
    string<N1, TL> const& l, const char (&r)[N2_1]) {
  return string<N1 + N2_1 - 1, typename array_upgrade<TL>::type>(
      l, string_literal<N2_1 - 1>(r), l.size(), N2_1 - 1);
}
